...
## Layout
    ...
    run_pipeline.py          rebuilds everything in ~2 minutes
    ...
    output/                  Excel model, figures, carousel
    data/                    pre-aggregated parquet (raw CSVs gitignored)
...
3. **The 25% substitution rate is an assumption**, reasoned from published academic-detailing results against a Part D generic dispensing rate already near 90%. It lives in an editable cell in the Excel model so anyone can substitute their own.
